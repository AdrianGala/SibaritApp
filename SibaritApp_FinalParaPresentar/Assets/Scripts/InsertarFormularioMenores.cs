...
public class InsertarFormularioMenores : MonoBehaviour
{
    ...
    public Text t_ErrorNombreA, t_ErrorApellido1A, t_ErrorApellido2A, t_ErrorDniA, t_ErrorDireccionA, t_ErrorTelefonoA, t_ErrorEmailA, t_ErrorFNacimientoA, t_ErrorNombreT, t_ErrorTerminos, t_ErrorCondiciones;
    ...
    public void ComprobacionEInsercion()
    {
        ...
        // Creo variables bool  para el adulto y el menor y uso la Regex.IsMatch que devuelve true si la variable cumple con el  patrón 
        bool isEmailAValid = Regex.IsMatch(emailAdulto, emailPattern);
        bool isnifARegExValid = Regex.IsMatch(dniAdulto, nifRegex);
        bool isPhoneAValid = Regex.IsMatch(telefonoAdulto, telefonoRegex);
        bool isnifMRegExValid = Regex.IsMatch(dniMenor, nifRegex);
        bool isPhoneMValid = Regex.IsMatch(telefonoMenor, telefonoRegex);
        ...
        // Recojo la fecha de hoy y la del campo calendario del adulto y el menor para poder comprobar si es mayor o menor a 18      
        int fechaSistemaA = System.DateTime.Today.Year;
        int fechaElegidaA = DateTime.Parse(fNacimientoAdulto).Year;
        int diferenciaFechasA = (fechaSistemaA - fechaElegidaA);
        int fechaSistemaM = System.DateTime.Today.Year;
        int fechaElegidaM = DateTime.Parse(fNacimientoMenor).Year;
        int diferenciaFechasM = (fechaSistemaM - fechaElegidaM);
        ...
        if (!dniAdulto.Equals("") && isnifARegExValid)
        {
            t_ErrorDniA.text = "";
        }
        else
        ...
        if (!telefonoAdulto.Equals("") && isPhoneAValid)
        {
        ...
        if (!isEmailAValid)
        {
            t_ErrorEmailA.text = "X";
        }
        else
        {
            t_ErrorEmailA.text = "";
        }
        if (fNacimientoAdulto.Equals("") || fNacimientoAdulto.Equals("00 / 00 / 0000") || diferenciaFechasA < 18)
        {
        ...
        if (!dniMenor.Equals("") && isnifMRegExValid)
        {
            t_ErrorDniM.text = "";
        }
        else
        ...
        if (!telefonoMenor.Equals("") && isPhoneMValid)
        {
        ...
        if (fNacimientoMenor.Equals("") || fNacimientoMenor.Equals("00 / 00 / 0000") || diferenciaFechasM > 18)
        {
